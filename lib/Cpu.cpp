...
int CPU::executeInstruction()
{
  ...
  if (0) {
	...
	char a[25];
	instructionToString(r.pc, a);
//	  printf("%04X: %-12s (%02X %02X %02X) A: %02X F: %s BC: %02X%02X DE: %02X%02X HL: %02X%02X\n",
//			 r.pc, a, inst,
//			 emu->memory.readByte(r.pc + 1), emu->memory.readByte(r.pc + 2), r.a, flags, r.b, r.c,
//			 r.d, r.e, r.h, r.l);
	printf("%04X: (%02X %02X %02X) A: %02X F: %s BC: %02X%02X DE: %02X%02X HL: %02X%02X\n",
		   r.pc, inst,
		   emu->memory.readByte(r.pc + 1), emu->memory.readByte(r.pc + 2), r.a, flags, r.b, r.c,
		   r.d, r.e, r.h, r.l);
  }
